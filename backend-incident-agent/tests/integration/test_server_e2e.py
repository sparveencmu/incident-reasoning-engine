...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    data = {
        ...
        "new_message": {
            "role": "user",
            "parts": [{"text": "{\"data\": {\"service\": \"cache-service\", \"severity\": \"LOW\", \"alert_name\": \"Cache eviction warning\", \"error_log\": \"Warning: Cache capacity is 85% full.\"}}"}],
        },
        "streaming": True,
    }
...
def test_pubsub_trigger(server_fixture: subprocess.Popen[str]) -> None:
    """Test the Pub/Sub trigger endpoint (/apps/app/trigger/pubsub)."""
    logger.info("Starting Pub/Sub trigger endpoint test")
    
    # Base64 encoding of:
    # {"service": "cache-service", "severity": "LOW", "alert_name": "Cache eviction warning", "error_log": "Warning: Cache capacity is 85% full."}
    encoded_data = "eyJzZXJ2aWNlIjogImNhY2hlLXNlcnZpY2UiLCAic2V2ZXJpdHkiOiAiTE9XIiwgImFsZXJ0X25hbWUiOiAiQ2FjaGUgZXZpY3Rpb24gd2FybmluZyIsICJlcnJvcl9sb2ciOiAiV2FybmluZzogQ2FjaGUgY2FwYWNpdHkgaXMgODUlIGZ1bGwuIn0="
    
    payload = {
        "message": {
            "data": encoded_data,
            "messageId": "12345",
            "publishTime": "2026-07-03T02:00:00Z"
        },
        "subscription": "projects/dummy-project/subscriptions/test-sub"
    }
    
    trigger_url = f"{BASE_URL}/apps/app/trigger/pubsub"
    response = requests.post(trigger_url, json=payload, headers=HEADERS, timeout=30)
    
    assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    logger.info(f"Pub/Sub trigger response status code: {response.status_code}")
    logger.info("Pub/Sub trigger test completed successfully")
